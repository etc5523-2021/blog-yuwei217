...
## Daily new cases and new death   
The table below is the record of daily new cases, new deaths, and new recoveries of Turkey from January 22, 2020, to August 31, 2021. You can search for situations on different days.
...
turkey <- covid19 %>% 
  ...
  rename(number=value) %>%
  arrange(date)
DT::datatable(turkey, options = list(pageLength= 3 ))
```
...
gt(turkey_vaccin) %>% 
  tab_header(title = "Summary of COVID-19 Epidemic in Turkey ",
...
gt(turkey_rate) %>% 
  tab_header(title = "Proportion Summary of COVID-19 Epidemic in Turkey ",
...
compare_rate1 <- compare_rate %>%
  ...
  DT::datatable(compare_rate1 , options = list(pageLength= 5 ))
```


If you play with the above table, you'd be surprised to learn that Turkey is doing pretty well during the epidemic. For fully vaccinated rate, Turkey can rank 29th within 207 countries. For case rate, Turkey ranks 49, while ranking 168 for the mortality rate. We can imagine that Turkey has made great efforts to deal with the outbreak. However, a better tomorrow requires the concerted efforts of all countries in the world.
